...
class AipOcr extends BaseClient {
    ...
    licensePlate(image, options) {
        let param = {image: image};
        let promise = this.registTask(this.licensePlateImpl, objectTools.merge(param, options));
        return promise;
    }
    licensePlateImpl(param) {
        let promise = new EventPromise();
        let httpClient = new HttpClient();
        let requestInfo = new RequestInfo(PATH_OCR_LICENSEPLATE,
            scope, param, METHOD_POST);
        if (this.preRequest(requestInfo)) {
            httpClient.postWithInfo(requestInfo).on(HttpClient.EVENT_DATA, function (data) {
                promise.resolve(data);
            ...
            return this.registTask(this.licensePlateImpl, param);
        }
        return promise;
    }
